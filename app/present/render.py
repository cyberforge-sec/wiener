...
def _metric_cards(bundle: EvidenceBundle) -> str:
    rows = []
    for mode in bundle.modes:
        metric = bundle.by_mode[mode]
        active = " metric-row--active" if mode == ExperimentMode.WIENER.value else ""
        width = max(0.0, min(100.0, (metric.uar or 0.0) * 100))
        rows.append(f'''<div class="metric-row{active}">
  <div class="metric-row-head"><span>{_esc(_mode_name(mode))}</span><code>{_pct(metric.uar)}</code></div>
...
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>WIENER — Authoritative Dashboard</title><link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&family=JetBrains+Mono:wght@400;500;600&display=swap" rel="stylesheet"><style>{_CSS}</style></head>
<body>{render_topbar('dashboard')}
<main class="wrap" data-environment="simulated" data-authoritative="true"><span class="sr-only">RED AI · SOC AGENT · BLUE AI · RISK ENGINE / POLICY GATE · METRICS</span>{_hero(bundle)}{_metric_cards(bundle)}{_pipeline(trial)}{_inspector(trial, bundle)}{_audit_layer(data, trial, bundle)}</main><script>{_JS}</script></body></html>'''
